...
ggplot(exam.per.student, aes(studied, correct.exam)) + 
  ...
  geom_boxplot(width=.4, outlier.color = NA, color="gray") +
...
# ggsave(file.path(figures.folder, "Proportion correct on exam f(studied).pdf"), width = 3.5, height = 5)
```
...
ggplot(new.data, aes(reps, pred.prop, group=alpha.N)) + 
  ...
  geom_line(aes(color=factor(alpha.N))) + 
  geom_point(aes(color=factor(alpha.N)), size=3, alpha=.9) + 
