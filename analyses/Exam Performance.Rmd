...
The simple approach: Is the probability of answering an item correctly higher if an item has been studied. The answer is yes: the estimated probability is `r percent(inv.logit(coef(m.stu)[1]))` of items that were not studied and `r percent(inv.logit(sum(coef(m.stu))))` for those that were. Note that these correspond very closely with the mean proportions in the plot above.
...
I'll use the model that includes both the `reps` and `final.alpha` to generate some model predictions that I can plot as a way to visualize the effects captured by the best-fitting model.

```{r}
# Refit the model with with alpha on the original scale:
m.noPC.noX.realAlpha <- glmer(correct.exam ~ 1 + final.alpha + reps.log + (1|Username) + (1|exam.Q), exam.item.studied, family = "binomial")

probs <- seq(.1, .9, .1)
# new.data <- data.frame(alpha.N = rep(quantile(exam.item.studied$alpha.N, probs = probs), each=length(probs)),
#                        reps.log = rep(quantile(exam.item.studied$reps.log, probs = probs), length(probs)))
# 
# new.data$pred <- predict(m.noPC.noX, newdata=new.data, re.form=NA)
new.data <- data.frame(final.alpha = rep(quantile(exam.item.studied$final.alpha, probs = probs), each=length(probs)),
                       reps.log = rep(quantile(exam.item.studied$reps.log, probs = probs), length(probs)))

new.data$pred <- predict(m.noPC.noX.realAlpha, newdata=new.data, re.form=NA)

# Convert to original scales:
new.data$pred.prop <- inv.logit(new.data$pred)
new.data$reps <- exp(new.data$reps.log)

ggplot(new.data, aes(reps, pred.prop, group=final.alpha)) + 
  # geom_line(aes(color=factor(alpha.N))) + 
  # geom_point(aes(color=factor(alpha.N)), size=3) + 
  geom_line(aes(color=final.alpha)) + 
  geom_point(aes(color=final.alpha), size=3) + 
  labs(x="\nRepetitions of an item", y=NULL, title="Predicted probability of answering an item correctly", color="Alpha:", subtitle="Predictions of best-fitting model for 10 to 90% quantiles of empirical data") + 
  scale_x_continuous(breaks = unique(new.data$reps), labels = round(unique(new.data$reps), 1)) +
  ylim(.5, 1) + 
  theme(legend.position = c(.9, .3)) +
  # scale_color_continuous(guide = guide_legend(label.theme = element_text(angle=30))) +
