...
ggsave(paste0(fig_path, "usage_by_day_of_course.pdf"), width = 5, height = 3)
```
...
ggsave(paste0(fig_path, "usage_by_hour_average.pdf"), width = 5, height = 3)
```
...
ggsave(paste0(fig_path, "usage_by_hour_per_day.pdf"), width = 5, height = 5)
...
ggsave(paste0(fig_path, "usage_by_hour_per_weekday_summed.pdf"), width = 7, height = 5)
...
ggsave(paste0(fig_path, "usage_by_hour_per_weekday_normalised.pdf"), width = 5, height = 5)
```
...
ggsave(paste0(fig_path, "session_count_histogram.pdf"), width = 5, height = 3)
...
ggsave(paste0(fig_path, "trial_count_histogram.pdf"), width = 5, height = 3)
```
...
ggsave(paste0(fig_path, "trials_per_session_histogram.pdf"), width = 5, height = 3)
...
ggsave(paste0(fig_path, "session_duration_histogram.pdf"), width = 5, height = 3)
...
ggplot(session_type_over_time, aes(x = day_of_course, y = n_sessions, colour = type)) +
  geom_line() +
  scale_x_continuous() +
  labs(x = "Day of course", y = "Number of session") +
  geom_vline(xintercept = c(rl_available_date - start_date, exam_date - start_date, resit_date - start_date), lty = "dotted") +
...
ggsave(paste0(fig_path, "usage_by_day_of_course_mc_vs_open.pdf"), width = 5, height = 3)
```
...
ggsave(paste0(fig_path, "session_count_by_user_and_type.pdf"), width = 5, height = 5)
```
...
ggsave(paste0(fig_path, "trial_count_by_user_and_type.pdf"), width = 5, height = 5)
