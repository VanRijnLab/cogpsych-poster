...
ggplot(session_type_over_time, aes(x = doy, y = n_sessions, colour = type)) +
  geom_line() +
...
The plots below show that both types of session tend to have about the same number of trials, but that sessions with multiple choice questions tend to take less time.
```{r}
...
boxplot(session_length_by_type$duration ~ session_length_by_type$type, main = "Session duration (min)")
```
