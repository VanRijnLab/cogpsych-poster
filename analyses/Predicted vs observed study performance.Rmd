...
```{r}
1 / (bf.prop <- proportionBF(sum(data$correct == data$est.correct), nrow(data), p=1/2))
```

The proportion test yields strong evidence that the proportion of instances in which the predicted response matches the observed response is equal to 0.5. It is in fact `r round(mean(data$correct == data$est.correct), 3)`. (The statistical test is based on N = `r prettyNum(nrow(data), big.mark=",")`, so there's a ton of data.)
...
# ggsave(file.path(figures.folder, "Predicted vs. observed RT distributions hexbin.pdf"), width = 5, height = 5)
```
...
Do the distributions look very different for trials with correct and incorrect responses?

```{r}
ggplot(data.RT.sub, aes(estimatedResponseTime)) + geom_density(aes(fill=correct), alpha=.6) + labs(title="Predicted RTs Only") + xlim(0, 20000)
```
