...
ggplot(dist, aes(x, group=type)) + 
  ...
  labs(title="Comparing RT Distributions: Observed vs. Predicted", x="Response Time (ms)", y=NULL, fill=NULL) +
...
It's difficult to get a good feeling for what a reasonable overlap/correlation/RMSE is for the predicted vs. observed RTs. A couple of things that should be noted, though: The RMSE is higher than the median. That seems quite high. The predicted distribution looks more normal than the observed: Its median is right at its peak, whereas it is shifted to the right of the peak of the observed distribution. RT distributions are usually skewed like that, not normal -- might point towards a general problem with predicted RTs that result in a normal distribution?
